...
bot = commands.Bot(help_command=None, command_prefix='$Sudo-', intents=intents, heartbeat_timeout=60)
TOKEN = os.getenv('DISCORD_TOKEN') 

Base = declarative_base()
db_directory = 'chathistory'

os.makedirs(db_directory, exist_ok=True)

engine = create_engine(f'sqlite:///{db_directory}/chat_history.db', echo=True)
...
if os.path.exists(DB_FILE_PATH) and len(bot.guilds) > MAX_HISTORY:
    os.remove(DB_FILE_PATH)
    print(f"Deleted {DB_FILE_PATH} due to exceeding MAX_HISTORY.")
    ...
    try:
        ...
        if is_active_channel or is_allowed_dm or contains_trigger_word or is_bot_mentioned or is_replied or bot_name_in_message:
            if string_channel_id in active_channels:
                instruc_config = active_channels[string_channel_id]  # INSTRUCTIONS CONFIG 
            else:
                instruc_config = config['INSTRUCTIONS']
            instructions = (
            ...
            async with message.channel.typing():
                ...
                if internet_access:
                    pass
                    # message.remove_reaction("", bot.user)
        ...
        logging.error(f"Error processing message: {message.content}. Error details: {e}")
...
async def clear(ctx):
    ...
    try:
        ...
        await ctx.send("⚠️ An error occurred while clearing message history", delete_after=2)
